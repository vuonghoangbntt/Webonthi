<?php
    $xtp = new Xtemplate("views/list_file.html");
    $class = $_GET['lop'];
    $class = $class-9;
    $sql = "SELECT T.test_id, T.test_title, T.test_description, U.user_name,T.test_date_post,V.cat_name, T.test_source
            ...
            WHERE T.user_id = U.user_id AND T.cat_id = V.cat_id AND T.type_id = 1 AND t.active = 1 AND V.cat_id = {$class}";
    ...
    foreach($data as $rs){
        ...
        if($flag!=0){
            ...
            <div class='col-md-4'>
                ...
                <button type='button' class='btn btn-primary'><span class='fas fa-pen' style='margin-right: 5px;'></span><a style='color: white;text-decoration: none;' href='?a=pre_test&id={$rs['test_id']}'>Làm online</a></button>
