<?php
    $xtp = new Xtemplate("views/comments/copo.html");
    // $sql = "SELECT * FROM tblcomment WHERE 1=1 ORDER BY com_id DESC";
    $sql = "SELECT * FROM tblcomment WHERE 1=1 ORDER BY comment_id DESC";
   ...
   // Tìm Trang hiện tại
    ...
    // $sqli = "SELECT * FROM tblcomment WHERE 1=1
    //         LIMIT {$start},{$limit}";
    $sqli = "SELECT C.comment_id,C.comment_content,C.comment_date_post,U.user_name,C.pos_id FROM tblcomment C INNER JOIN tbluser U ON C.user_id = U.user_id ORDER BY comment_id DESC
            LIMIT {$start},{$limit}";
